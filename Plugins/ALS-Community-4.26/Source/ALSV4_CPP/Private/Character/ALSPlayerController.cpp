...
void AALSPlayerController::StartRotateCamera(const FInputActionValue& Value)
{
	bIsRotatingCameraRMB = true;

	// Hide cursor while rotating with RMB
...
void AALSPlayerController::StopRotateCamera(const FInputActionValue& Value)
{
	bIsRotatingCameraRMB = false;

	// If LMB is NOT down either, restore cursor + UI input
...
void AALSPlayerController::StartFreeLook(const FInputActionValue& Value)
{
	bIsRotatingCameraLMB = true;
	bIsFreeLook = true;

	// Hide cursor while freelooking
	bShowMouseCursor = false;
	...
	if (PossessedCharacter)
	{
		// Temporarily stop character from following controller yaw
		bCachedUseControllerRotationYaw = PossessedCharacter->bUseControllerRotationYaw;
		PossessedCharacter->bUseControllerRotationYaw = false;
	}
}

void AALSPlayerController::StopFreeLook(const FInputActionValue& Value)
{
	bIsRotatingCameraLMB = false;
	bIsFreeLook = false;

	if (PossessedCharacter)
	{
		// Restore original rotation behavior
		PossessedCharacter->bUseControllerRotationYaw = bCachedUseControllerRotationYaw;
	}

	// If RMB is not down either, bring cursor back
	if (!IsCameraRotationActive())
	{
		bShowMouseCursor = true;
...
void AALSPlayerController::CameraUpAction(const FInputActionValue& Value)
{
	...
	// Only rotate camera when either RMB or LMB is held
	if (!IsCameraRotationActive())
	{
		return;
	}
...
void AALSPlayerController::ZoomCameraAction(const FInputActionValue& Value)
{
	...
	if (AALSPlayerCameraManager* CamMgr = Cast<AALSPlayerCameraManager>(PlayerCameraManager))
	{
		CamMgr->AddCameraZoom(AxisValue);
	}
}
